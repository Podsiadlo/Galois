...
protected:
  ...
  static void logg(const NodeData& interiorData,
    ...
    std::cout << "interior: (" << interiorData.getPoints().toString()
              << "), neighbours: (";
    for (auto vertex : verticesData) {
      std::cout << vertex.getPoints().toString() + ", ";
    }
    std::cout << ") ";
  }
...
private:
  ...
  GNode createNodeOnEdge(int edgeToBreak, ProductionState& pState,
    ...
    const Coordinates& newPointCoords = getNewPointCoords(
        pState.getVerticesData()[edgeVertices.first].getPoints(),
        pState.getVerticesData()[edgeVertices.second].getPoints(),
        pState.getZGetter());
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      graph.getEdgeData(edge).setMiddlePoint(
          (newNodeData.getPoints().getX() + vertexData.getPoints().getX()) / 2.,
          (newNodeData.getPoints().getY() + vertexData.getPoints().getY()) / 2.,
          (newNodeData.getPoints().getZ() + vertexData.getPoints().getZ()) /
              2.);
      // distance
      graph.getEdgeData(edge).setLength(newNodeData.getPoints().dist(
          vertexData.getPoints(), pState.isVersion2D()));
    }
    return newNode;
  }
  ...
  void breakElementUsingNode(int edgeToBreak, GNode const& hangingNode,
    ...
    double length      = 0;
    length             = hNodeData.getPoints().dist(
        pState.getVerticesData()[neutralVertex].getPoints(),
        pState.isVersion2D());
