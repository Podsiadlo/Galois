...
private:
  void fixTriangle(int toBreak, const Coordinates& hangingPoint,
                   vector<Edge> edges, vector<GNode> gNodes, GNode triangle) {
    Coordinates& oppositePoint =
        Edge::getCommonPoint(edges[firstOpposite(toBreak)],
                             edges[secondOpposite(toBreak)])
            .get();
    GNode newNode =
        graph->createAndAddNode(Edge(oppositePoint, hangingPoint, version2D));
    ...
    const vector<GNode>& edgeHalves = graph->getGNodesFrom(gNodes[toBreak]);
  ...
  Coordinates breakEdge(int toBreak, Bag* bag, const vector<Edge>& edges,
                        const vector<GNode>& gNodes) {
    Edge edgeToBreak = edges[toBreak];
    edgeToBreak.setBroken(true);
    Coordinates& middle = bag->emplace_back(edgeToBreak.getMiddle());
    Edge newEdge1(edgeToBreak.getNodes().first, std::cref(middle), version2D);
    Edge newEdge2(std::cref(middle), edgeToBreak.getNodes().second, version2D);
  ...
  int chooseEdge(const Edge& triangle, const vector<Edge>& edges) {
